...
def load_selectors(root_dir):
    selectors_path = pkl_to_json(st.load.get_selectors_path(root_dir))
    _dict = safe_load_json(selectors_path)
    return {k: torch.tensor(v) for k, v in _dict.items()}


def load_scalers(root_dir):
    scalers_path = pkl_to_json(st.load.get_scalers_path(root_dir))
    _dict = safe_load_json(scalers_path)
    result = {}
    for k in _dict:
        scaler = st.scale.SequentialScaler()
...
def load_hps(root_dir):
    hps_path = pkl_to_txt(st.load.get_hps_path(root_dir))
...
def load_features(root_dir):
    features_path = pkl_to_txt(st.load.get_features_path(root_dir))
